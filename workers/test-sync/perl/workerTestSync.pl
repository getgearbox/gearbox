...
my $worker = Gearbox::WorkerTestSync->new( $ARGV[0] );
$worker->run();

package Gearbox::WorkerTestSync;
...
sub new {
    ...
    $self->register_handler( "do_get_testsyncperl_thing_v1" );
    $self->register_handler( "do_put_testsyncperl_thing_v1" );
    $self->register_handler( "do_post_testsyncperl_thing_v1" );
    $self->register_handler( "do_delete_testsyncperl_thing_v1" );

    return $self;
}

sub do_get_testsyncperl_thing_v1 {
    my $self = shift;
    my $job  = shift;
    my $resp = shift;

    my $environ = $job->environ();
    if ( exists $environ->{"TestSync"} ) {
        $resp->add_header( "TestSync", $environ->{"TestSync"} );
    }
...
sub do_put_testsyncperl_thing_v1 {
    my $self = shift;
    my $job  = shift;
    my $resp = shift;
...
sub do_post_testsyncperl_thing_v1 {
    my $self = shift;
    my $job  = shift;
    my $resp = shift;
...
sub do_delete_testsyncperl_thing_v1 {
    my $self = shift;
    my $job  = shift;
    my $resp = shift;
